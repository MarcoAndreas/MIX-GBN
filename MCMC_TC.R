MCMC_TC <- function(data_con, VEC_DIS, incidence, iterations, step_save){
  ...
  ################################################################################
  ###  log BGe Score of the FIRST graph
 
  OUT = GENERATE_PARA_LIST_TC(VEC_DIS,data_con,T_0,v,mu)
  ...
  out = COMPUTE_BGE_TC(T_m_LIST, m_LIST, m, incidence, a, v, T_0)
  ...
  for (z in 2:((iterations/step_save)+1)){
    ...
    for (count in 1:step_save){
      ...
      out = COMPUTE_BGE_LOCAL_TC(operation,child_node,parent_node,T_m_LIST,m_LIST,m,P_local_num,P_local_den,incidence_new,a,v,T_0)
    ...
    for (k in 1:K){
     ...
     OUT = GENERATE_PARA_LIST_TC(VEC_DIS_NEW,data_con,T_0,v,mu)
     MEANS_LIST = OUT[[1]]
     T_m_LIST   = OUT[[2]]
     m_LIST     = OUT[[3]]
     out = COMPUTE_BGE_TC(T_m_LIST, m_LIST, m, incidence, a, v, T_0)
